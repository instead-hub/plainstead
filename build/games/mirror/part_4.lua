...
--    _assailant = true,
    ...
    dsc = function(s)
        return 'Нижняя часть башни утопала в холодном полумраке, создаваемом тусклыми факелами на стенах. На длинных цепях, тянувшихся с самого потолка башни, висело большое колесо старой люстры.';
    end,
...
kolodec = iobj {
    nam = 'колодец',
	dsc = function(s)
        return 'Посреди комнаты находился {колодец}, окружённый каменной кладкой.';
	end,
	exam = 'Внимательно изучив колодец, я обнаружил что он полон чистой воды.',
    useit = function (s, w)
		status._health = status._health + 20;
		return 'Я набрал в руки горсть прохладной воды. Живительная влага сняла усталость, я стал чувствовать себя лучше.';
    end,
	talk = function(s)
        return 'Когда человек начинает разговаривать с колодцами, то ему следует подумать о своём душевном здоровье...';
    end,
};
...
castle_405_inscription_room = room {
    ...
    dsc = function(s)
        return 'На плите была надпись на неизвестном мне языке: ЧШРСЛР Й ФРШ ЪМХМС, ПЗКУЖХР Й. Нижняя часть плиты была отбита.';
    end,
...
castle_419_bad_end = room {
    ...
    dsc = function(s)
        return 'Но вдруг я почувствовал, как поток энергии накрывает чёрная сила. Маг направил действие своих чар на меня, усиливая их Священным Огнём. Зло ворвалось в моё сознание, заполняя и поглощая его. И через некоторое время я был полностью во власти тёмных сил...^...Но почему зло? Истинное зло — то, которое кроется в каждом человеке, а не то, которое люди называют «злом»! Чтобы уничтожить его, необходимо уничтожить всех людей. Я властен над Демоном Войны! Я сделаю это!!!';
    end,
    obj = { vway('1', '{Далее}', 'The_End') },
--    way = { vroom ('Тоннель', 'castle_417'), vroom ('На свет', 'castle_419') },
};
...
final_02 = room {
    ...
    dsc = function(s)
        return 'Тёмный властелин с трона говорит: Ты преодолел все преграды, что я ставил на твоём пути.';
    end,
...
final_03 = room {
    ...
    dsc = function(s)
		return 'Но, хранитель, тебе не сломить меня! Человек не властен над Чёрным Магом.';
    end,
...
final_04 = room {
    ...
    dsc = function(s)
		return 'Ты прав, - смело отвечает хранитель, - я не смогу сражаться с тобой. Но Демон Войны остановит твои злодеяния!';
    end,
...
final_05 = room {
    ...
    dsc = function(s)
        return 'В зеркале появляется силуэт могучего Демона.';
    end,
...
--        return 'Авторы оригинальной игры^Творческая группа Art Work, 1998 год^^Сценарий и графика — Степанов В. Ю.^^Коды и музыка — Пасечник И. А.^^Адаптация для INSTEAD — Вадим В. Балашов.^vvb.backup@rambler.ru^^Автор выражает огромную признательность Петру Косых за INSTEAD и неоценимую помощь, оказанную при разработке кода.';
        return 'Авторы оригинальной игры —^Творческая группа Art Work, 1998 год^^Сценарий и графика — Степанов В. Ю.^^Коды и музыка — Пасечник И. А.^^^Адаптация для INSTEAD — Вадим В. Балашов.^vvb.backup@rambler.ru^^Автор выражает огромную признательность Петру Косых за INSTEAD и неоценимую помощь, оказанную при разработке кода. Адаптация для PlainIntead - Ласточкин А. П., тестирование игры - Гаджимурадов А. И.';
    end,
...
castle_424_matrix_view = room {
    ...
    dsc = function(s)
        return 'Я поднялся немного по лестнице и посмотрел сверху на пол. Ровные линии образовывали двенадцать плит, на каждой из которых были изображены непонятные знаки. Первая строка: М Т Х Й. Вторая строка: П У З Ш. Третья строка: Ч Р Ъ Ц.';
    end,
    exit = function (s)
        me():enable_all();
    end,
...
plate_e = iobj {
    _occupied = false,
    nam = function(s)
        local v = 'плита М'..img ('images/plate_e.png');
        return v;
    end,
    dsc = function (s)
        return true;
    ...
    used = function (s, w)
        if s._occupied then
            local v = 'Плита М'..img ('images/plate_e.png')..' уже занята.';
            return v;
        end
        if not have (w) then
            ...
            Drop ( ref(w) );
            s._occupied = true;
            local v = 'Я положил '..nameof(w)..' на плиту М'..img ('images/plate_e.png')..'.';
            local r = check_floor ();
            if r then
                return v..r;
            else
...
plate_k = iobj {
    _occupied = false,
    nam = function(s)
        local v = 'плита Т'..img ('images/plate_k.png');
        return v;
    end,
    dsc = function (s)
        return true;
    ...
    used = function (s, w)
        if s._occupied then
            local v = 'Плита Т'..img ('images/plate_k.png')..' уже занята.';
            return v;
        end
        if not have (w) then
            return 'Чтобы положить предмет на плиту, надо держать его в руках.';
            ...
            Drop ( ref(w) );
            s._occupied = true;
            local v = 'Я положил '..nameof(w)..' на плиту Т'..img ('images/plate_k.png')..'.';
            local r = check_floor ();
            if r then
                return v..r;
            else
...
plate_n = iobj {
    _occupied = false,
    nam = function(s)
        local v = 'плита Х'..img ('images/plate_n.png');
        return v;
    end,
    dsc = function (s)
        return true;
    ...
    used = function (s, w)
        if s._occupied then
            local v = 'Плита Х'..img ('images/plate_n.png')..' уже занята.';
            return v;
        end
        if not have (w) then
            return 'Чтобы положить предмет на плиту, надо держать его в руках.';
            ...
            Drop ( ref(w) );
            s._occupied = true;
            local v = 'Я положил '..nameof(w)..' на плиту Х'..img ('images/plate_n.png')..'.';
            local r = check_floor ();
            if r then
                return v..r;
            else
...
plate_v = iobj {
    _occupied = false,
    nam = function(s)
        local v = 'плита Й'..img ('images/plate_v.png');
        return v;
    end,
    dsc = function (s)
        return true;
    ...
    used = function (s, w)
        if s._occupied then
            local v = 'Плита Й'..img ('images/plate_v.png')..' уже занята.';
            return v;
        end
        if not have (w) then
            return 'Чтобы положить предмет на плиту, надо держать его в руках.';
            ...
            Drop ( ref(w) );
            s._occupied = true;
            local v = 'Я положил '..nameof(w)..' на плиту Й'..img ('images/plate_v.png')..'.';
            local r = check_floor ();
            if r then
                return v..r;
            else
...
plate_z = iobj {
    _occupied = false,
    nam = function(s)
        local v = 'плита П'..img ('images/plate_z.png');
        return v;
    end,
    dsc = function (s)
        return true;
    ...
    used = function (s, w)
        if s._occupied then
            local v = 'Плита П'..img ('images/plate_z.png')..' уже занята.';
            return v;
        end
        if not have (w) then
            return 'Чтобы положить предмет на плиту, надо держать его в руках.';
            ...
            Drop ( ref(w) );
            s._occupied = true;
            local v = 'Я положил '..nameof(w)..' на плиту П'..img ('images/plate_z.png')..'.';
            local r = check_floor ();
            if r then
                return v..r;
            else
...
plate_l = iobj {
    _occupied = false,
    nam = function(s)
        local v = 'плита У'..img ('images/plate_l.png');
        return v;
    end,
    dsc = function (s)
        return true;
    ...
    used = function (s, w)
        if s._occupied then
            local v = 'Плита У'..img ('images/plate_l.png')..' уже занята.';
            return v;
        end
        if not have (w) then
            return 'Чтобы положить предмет на плиту, надо держать его в руках.';
            ...
            Drop ( ref(w) );
            s._occupied = true;
            local v = 'Я положил '..nameof(w)..' на плиту У'..img ('images/plate_l.png')..'.';
            local r = check_floor ();
            if r then
                return v..r;
            else
...
plate_a = iobj {
    _occupied = false,
    nam = function(s)
        local v = 'плита З'..img ('images/plate_a.png');
        return v;
    end,
    dsc = function (s)
        return true;
    ...
    used = function (s, w)
        if s._occupied then
            local v = 'Плита З'..img ('images/plate_a.png')..' уже занята.';
            return v;
        end
        if not have (w) then
            return 'Чтобы положить предмет на плиту, надо держать его в руках.';
            ...
            Drop ( ref(w) );
            s._occupied = true;
            local v = 'Я положил '..nameof(w)..' на плиту З'..img ('images/plate_a.png')..'.';
            local r = check_floor ();
            if r then
                return v..r;
            else
...
plate_r = iobj {
    _occupied = false,
    nam = function(s)
        local v = 'плита Ш'..img ('images/plate_r.png');
        return v;
    end,
    dsc = function (s)
        return true;
    ...
    used = function (s, w)
        if s._occupied then
            local v = 'Плита Ш'..img ('images/plate_r.png')..' уже занята.';
            return v;
        end
        if not have (w) then
            return 'Чтобы положить предмет на плиту, надо держать его в руках.';
            ...
            Drop ( ref(w) );
            s._occupied = true;
            local v = 'Я положил '..nameof(w)..' на плиту Ш'..img ('images/plate_r.png')..'.';
            local r = check_floor ();
            if r then
                return v..r;
            else
...
plate_p = iobj {
    _occupied = false,
    nam = function(s)
        local v = 'плита Ч'..img ('images/plate_p.png');
        return v;
    end,
    dsc = function (s)
        return true;
    ...
    used = function (s, w)
        if s._occupied then
            local v = 'Плита Ч'..img ('images/plate_p.png')..' уже занята.';
            return v;
        end
        if not have (w) then
            return 'Чтобы положить предмет на плиту, надо держать его в руках.';
            ...
            Drop ( ref(w) );
            s._occupied = true;
            local v = 'Я положил '..nameof(w)..' на плиту Ч'..img ('images/plate_p.png')..'.';
            local r = check_floor ();
            if r then
                return v..r;
            else
...
plate_i = iobj {
    _occupied = false,
    nam = function(s)
        local v = 'плита Р'..img ('images/plate_i.png');
        return v;
    end,
    dsc = function (s)
        return true;
    ...
    used = function (s, w)
        if s._occupied then
            local v = 'Плита Р'..img ('images/plate_i.png')..' уже занята.';
            return v;
        end
        if not have (w) then
            return 'Чтобы положить предмет на плиту, надо держать его в руках.';
            ...
            Drop ( ref(w) );
            s._occupied = true;
            local v = 'Я положил '..nameof(w)..' на плиту Р'..img ('images/plate_i.png')..'.';
            local r = check_floor ();
            if r then
                return v..r;
            else
...
plate_t = iobj {
    _occupied = false,
    nam = function(s)
        local v = 'плита Ъ'..img ('images/plate_t.png');
        return v;
    end,
    dsc = function (s)
        return true;
    ...
    used = function (s, w)
        if s._occupied then
            local v = 'Плита Ъ'..img ('images/plate_t.png')..' уже занята.';
            return v;
        end
        if not have (w) then
            return 'Чтобы положить предмет на плиту, надо держать его в руках.';
            ...
            Drop ( ref(w) );
            s._occupied = true;
            local v = 'Я положил '..nameof(w)..' на плиту Ъ'..img ('images/plate_t.png')..'.';
            local r = check_floor ();
            if r then
                return v..r;
            else
...
plate_o = iobj {
    _occupied = false,
    nam = function(s)
        local v = 'плита Ц'..img ('images/plate_o.png');
        return v;
    end,
    dsc = function (s)
        return true;
    ...
    used = function (s, w)
        if s._occupied then
            local v = 'Плита Ц'..img ('images/plate_o.png')..' уже занята.';
            return v;
        end
        if not have (w) then
            return 'Чтобы положить предмет на плиту, надо держать его в руках.';
            ...
            Drop ( ref(w) );
            s._occupied = true;
            local v = 'Я положил '..nameof(w)..' на плиту Ц'..img ('images/plate_o.png')..'.';
            local r = check_floor ();
            if r then
                return v..r;
            else
...
castle_425_table_1 = room {
    ...
    dsc = function(s)
        return 'На таблице была высечена странная надпись: ЛИШЬ ОДЕРЖАВ ПОБЕДУ НАД СОБОЙ ДУХ ОБРЕТЕТ УТЕРЯННЫЙ ПОКОЙ';
    end,
...
castle_425_table_2 = room {
    ...
    dsc = function(s)
        return 'На таблице была высечена странная надпись на незнакомом языке. Её текст: ЦЪТШЦМЪ ЩУЦЙЦ ЧЫЪД Т ЦКХЁ, ШЗПЙММЪ ЧУЗФЖ ЯЗШГ ЪДФГ, ФРШ ЦЩЪЗХЦЙРЪ ЩЪЗШЫЁ ЙЦСХЫ, Р ПУЦ ЫСЛМЪ Й ПМШТЗУДХГМ ФРШГ.';
    end,
    obj = { vway('1', '{Далее}', 'castle_425') },
    exit = function (s, from)
        me():enable_all();
...
castle_425_table_3 = room {
    ...
    dsc = function(s)
        return 'На таблице была высечена странная надпись: УРАД ЦЛМШОЗЙ ЧЦИМЛЫ ХЗЛ ЩЦИЦС ЛЫЭ ЦИШМЪМЪ ЫЪМШЖХХГС ЧЦТЦС.';
    end,
    obj = { vway('1', '{Далее}', 'castle_425') },
    exit = function (s, from)
        me():enable_all();
