...
--			Фикс - если есть гриб на плоту, не уменьшает массу груза (закомментил Drop, раскомментил inv():del)
			Drop (mushroom);
            objs():del(mushroom);
--          inv():del(mushroom);
...
labyrinth_45 = room {
    ...
    dsc = function(s)
	    if spider._is_alive then
			...
			return 'Вскарабкавшись по липкой паутине наверх, я оказался в самом логове чудовища, построившего эту нору. К счастью, его бывший владелец уже не опасен.';
		end
    end,
    rest = 'Я присел на пол и немного отдохнул.',
    obj = { 'spider', 'web_45' },
