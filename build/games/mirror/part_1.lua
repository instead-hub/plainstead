    ...
    used = function (s, w)
        if w == plate then
            if have (plate) then
                if s._chain_on then
                    s._chain_on = false;
                    return 'Превозмогая отвращение, я зачерпнул немного жира из тарелки и смазал им кисти рук. Плохо подогнанные кандалы с лёгкостью соскользнули на пол.';
                else
...
The_End = room {
	...
	dsc = 'Теперь никто не помешает мне править миром! - сказал Чёрный властелин',
    enter = function (s)
